...
```
# Review overview
```{r review_overview, cache = TRUE, cache.lazy = FALSE, cache.extra = mtime(here("output", "review_processed.Rdata"))}
...
```{r plot_reviews, include = TRUE, cache = TRUE, cache.lazy = FALSE, cache.extra = mtime(here("output", "str_processed.qsm"))}
...
```{r fake_review_identification, cache = TRUE, cache.lazy = FALSE, cache.extra = mtime(here("output", "fake_reviews_uc.Rdata"))}
...
```{r fake_review_location, include = TRUE, echo = FALSE, warning = FALSE}
...
```{r fake_review_commercial_operations, cache = TRUE, cache.lazy = FALSE, cache.extra = mtime(here("output", "str_processed.qsm"))}
...
```{r fake_review_impacts, include = TRUE, cache = TRUE, cache.lazy = FALSE, cache.extra = mtime(here("output", "str_processed.qsm"))}
...
rev_impacts_matrice_indexed <- 
rev_impacts_matrice %>% 
  mutate(index = case_when(date_rev-months(1) == date_daily ~ "Month before review",
                           date_rev == date_daily ~ "Month of review",
                           date_rev+months(1) == date_daily ~ "Month after review",
                           TRUE ~ "filter_out"
  )) %>% 
  filter(index != "filter_out") %>% 
  mutate(days_active = sum(n)) %>% 
  ungroup() %>% 
  filter(status == "R") %>% 
  rename(days_res = n) %>% 
  group_by(index) %>% 
  summarize(per_res = mean(days_res/days_active))
  
rev_impacts_matrice_indexed %>% 
  ggplot()+
  geom_point(aes(factor(index, level = c("Month before review", "Month of review", "Month after review")), per_res))+
  scale_y_continuous(limits = c(0.64, 0.73), labels = scales::percent)+
  ggtitle("Impact of fake reviewing on listings' percentage \nof reserved active days")+
  xlab(NULL)+
...
```
Previously is a figure presenting the percentage of reserved days out of all active days (either available or active) during the month prior to a fake review, the month of the review, and a month after it. On average, a fake review is let during a month of lower percentage of reserved active days compared to the precedent, but only in a marginal manner (difference of only `r scales::percent(pull(rev_impacts_matrice_indexed[2,2] - rev_impacts_matrice_indexed[3,2]), accuracy = 0.1)`). There is a significant spike for this statistic the month following a fake review, which passes from `r scales::percent(pull(rev_impacts_matrice_indexed[2,2]), accuracy = 0.1)` to `r scales::percent(pull(rev_impacts_matrice_indexed[1,2]), accuracy = 0.1)`, a difference of `r scales::percent(pull(rev_impacts_matrice_indexed[1,2] - rev_impacts_matrice_indexed[2,2]), accuracy = 0.1)`. 

```{r fake_review_by_month_impacts, include = TRUE, cache = TRUE, cache.lazy = FALSE, cache.extra = mtime(here("output", "str_processed.qsm"))}

qload(here("output", "str_processed.qsm"),
      nthreads = parallel::detectCores()-1)

fake_review_impacts_months <- 
  rev_impacts_matrice %>% 
...
```
Commercial listings who forge reviews have a higher percentage of reserved active days. The first two months of a listing is where the growth from month to month is the highest: listings gained reputation and with enough ratings and reviews, more reservations come. This is a normal characteristics of every listings. However, in the first two months of operation, listings who faked reviews had much higher percentage of reserved days. Fake reviewing during the first two months of operation seems to be very advantageous: they alleviate the problem of cold start, when a listing has no reviews so has a harder time to get reserved. The vertical distance between the fake reviewed listings' lines and the rest of the commercial listings shows the advantage of this fraudulent behavior.
