...
property %>% 
  ...
  mutate(matched = glue::glue("{matched} listings matched"),
...
property %>% 
  ...
  slice(1:6) %>% 
  select(ab_image_url, matched, city) %>% 
  mutate(matched = glue::glue("{matched} listings matched in {city}"),
         ab_image_url = str_sub(ab_image_url, start=1, end=-20))
